...
# copied from activesupport-6.1.5
...
class Module
  ...
  def mattr_reader(*syms, instance_reader: true, instance_accessor: true, default: nil,
    ...
    definition = []
    syms.each do |sym|
      fail NameError, "invalid attribute name: #{sym}" unless /\A[_A-Za-z]\w*\z/.match?(sym)

      definition << "def self.#{sym}; @@#{sym}; end"

      definition << "def #{sym}; @@#{sym}; end" if instance_reader && instance_accessor

      sym_default_value = block_given? && default.nil? ? yield : default
      unless sym_default_value.nil? && class_variable_defined?("@@#{sym}")
  ...
  def mattr_writer(*syms, instance_writer: true, instance_accessor: true, default: nil,
    ...
    syms.each do |sym|
      ...
      definition << "def #{sym}=(val); @@#{sym} = val; end" if instance_writer && instance_accessor
